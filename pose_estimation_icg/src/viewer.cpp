...
namespace icg
{
    ...
    void Viewer::DisplayAndSaveImage (int save_index, const cv::Mat &image)
    {
        if (display_images_){
            ...
            cv::Mat roi = image(cv::Rect(0, image.rows - 100, 640, 100));
            cv::Mat color(roi.size(), CV_8UC3, cv::Scalar(255,255,255));
            cv::addWeighted(color, 0.5, roi, 1.0 - 0.5, 0.0, roi);
            
            cv::putText(image,
                    "Press 'd' to start manual initialization.",
                    cv::Point(10, image.rows - 70), 
                    cv::FONT_HERSHEY_DUPLEX, 1, CV_RGB(0, 0, 0), 1);
            cv::putText(image,
                    "Press 't' to start/continue tracking.",
                    cv::Point(10, image.rows - 45), 
                    cv::FONT_HERSHEY_DUPLEX, 1, CV_RGB(0, 0, 0), 1);
            cv::putText(image,
                    "Press 's' to pause tracking.",

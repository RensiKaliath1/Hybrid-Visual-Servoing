...
namespace icg
{
                                                  ...
                                                  int corr_iteration)
    {
        ...
        for (int j = 0; j < 2; ++j)
        {
            ...
            for (int i = 0; i < n_points_; ++i)
            {
                ...
                std::stringstream ss;
                ss << data_model_points[i].center_f_body;
                std::cout << ss.str() << std::endl; 
                ss << data_model_points[i].normal_f_body;
                std::cout << ss.str() << std::endl; 
            ...
            if (data_points_.size() >= min_n_unoccluded_points_)
                break;
        }
        return true;
    }
                                      ...
                                      bool model_occlusions) const
    {
        ...
        if (data_point.center_f_camera (2) <= 0.0f)
            return false;
        ...
        {
            if (!IsPointUnoccludedMeasured (data_point))
                return false;
        }
        ...
        if (model_occlusions)
        {
            if (!IsPointUnoccludedModeled (data_point))
                return false;
        }
        return true;
    }

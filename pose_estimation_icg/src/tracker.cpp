...
namespace icg
{
    ...
    bool Tracker::RunTrackerProcess(bool execute_detection, bool start_tracking)
    {
        ...
        for (int iteration = 0;; ++iteration)
        {
            ...
            {
                if (!ExecuteDetectionCycle(iteration))
                    return false;
                tracking_started_ = false;
            ...
            {
                if (!ExecuteTrackingCycle(iteration))
                    return false;
            }
            if (!UpdateViewers(iteration))
                return false;
            if (quit_tracker_process_)
                return true;
            if (!synchronize_cameras_)
                WaitUntilCycleEnds(begin);
        }
        return true;
    }
    ...
    bool Tracker::StartModalities(int iteration)
    {
        ...
        {
            if (!start_modality_renderer_ptr->StartRendering())
                return false;
        }
        for (auto &modality_ptr : modality_ptrs_)
        {
            if (!modality_ptr->StartModality(iteration, 0))
                return false;
        }
        return true;
    }
    ...
    bool Tracker::ExecuteTrackingCycle(int iteration)
    {
             ...
             ++corr_iteration)
        {
            ...
            if (!CalculateCorrespondences(iteration, corr_iteration))
                return false;
            if (!VisualizeCorrespondences(corr_save_idx))
                return false;
                 ...
                 ++update_iteration)
            {
                ...
                if (!CalculateOptimization(iteration, corr_iteration, update_iteration))
                    return false;
                if (!VisualizeOptimization(update_save_idx))
                    return false;
            }
        }
        if (!CalculateResults(iteration))
            return false;
        if (!VisualizeResults(iteration))
            return false;
    ...
    bool Tracker::UpdateCameras(bool update_all_cameras)
    {
        if (update_all_cameras)
        {
            ...
            {
                if (!camera_ptr->UpdateImage(synchronize_cameras_))
                    return false;
            }
        }
        else
    ...
    bool Tracker::CalculateCorrespondences(int iteration, int corr_iteration)
    {
        ...
        {
            if (!correspondence_renderer_ptr->StartRendering())
                return false;
        }
        for (auto &modality_ptr : modality_ptrs_)
        {
    ...
    bool Tracker::VisualizeCorrespondences(int save_idx)
    {
        ...
        for (auto &modality_ptr : modality_ptrs_)
        {
            if (!modality_ptr->VisualizeCorrespondences(save_idx))
                return false;
            if (modality_ptr->imshow_correspondence())
                imshow_correspondences = true;
        }
        if (imshow_correspondences)
        {
            if (cv::waitKey(visualization_time_) == 'q')
                return false;
        }
        return true;
    }
    ...
    bool Tracker::VisualizeOptimization(int save_idx)
    {
        ...
        for (auto &modality_ptr : modality_ptrs_)
        {
            if (!modality_ptr->VisualizeOptimization(save_idx))
                return false;
            if (modality_ptr->imshow_optimization())
                imshow_pose_update = true;
        }
    ...
    bool Tracker::CalculateResults(int iteration)
    {
        ...
        {
            if (!results_renderer_ptr->StartRendering())
                return false;
        }
        for (auto &modality_ptr : modality_ptrs_)
        {
            if (!modality_ptr->CalculateResults(iteration))
                return false;
        }
        return true;
    }
    ...
    bool Tracker::VisualizeResults(int save_idx)
    {
        ...
        for (auto &modality_ptr : modality_ptrs_)
        {
            if (!modality_ptr->VisualizeResults(save_idx))
                return false;
            if (modality_ptr->imshow_result())
                imshow_result = true;
        }
        if (imshow_result)
        {
    ...
    bool Tracker::UpdatePublishers(int iteration)
    {
        ...
        {
            if (!publisher_ptr->UpdatePublisher(iteration))
                return false;
        }
        return true;
    }
    ...
    const std::vector<std::shared_ptr<RendererGeometry>> &Tracker::renderer_geometry_ptrs() const
    {
    ...
    const std::vector<std::shared_ptr<Renderer>> &Tracker::start_modality_renderer_ptrs() const
    {
        return start_modality_renderer_ptrs_;
    }

    const std::vector<std::shared_ptr<Renderer>> &Tracker::correspondence_renderer_ptrs() const
    {

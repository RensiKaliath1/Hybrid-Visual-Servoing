...
bool Optimizer::UpdatePoses(const std::shared_ptr<Link> &link_ptr,
  ...
  for (auto &child_link_ptr : link_ptr->child_link_ptrs())
   ...
   std::cout<<"\n link to world pose of "<< link_ptr.get()->body_ptr().get()->get_name() <<":  "<<link_ptr.get()->body_ptr().get()->body2world_pose().translation().x()<<" "<<link_ptr.get()->body_ptr().get()->body2world_pose().translation().y()<<" "<<link_ptr.get()->body_ptr().get()->body2world_pose().translation().z();
   return true;
}
